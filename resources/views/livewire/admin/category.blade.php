    ...
    <main class="bg-light">
        <div class="container-fluid px-4 ">
                            ...
                            <div class="custom-file">
                                ...
                                @if ($edit_thumbnail)
                                    ...
                                    <img width="40%" height="40%"
                                        src="{{ config('app.url'). $edit_thumbnail }}">
                                @endif
            ...
            <div class="row mt-4 shadow rounded p-3">
                <div class="col mx-auto">
                    <h2 class="text-success rounded text-center shadow p-3 bg-info text-light border border-success">All Categories</h2>
                    <table class="table table-all table-hoverable">
                        ...
                        @forelse($categories as $category)
                            <tr>
                                ...
                                <td><img width="50px" height="50px" src="{{ config('app.url'). $category->thumbnail }}" alt="image"></td>
                                <td>{{ $category->created_at->format('d-m-y') }}</td>
                                <td>
                                    <div class="btn-group">
                                        <button class="btn btn-sm btn-warning" title="edit this row" wire:click="edit({{$category->id}})">Edit</button>
                                        <button class="btn btn-sm btn-danger" title="delete this row" onclick="return confirm('{{ __('Are You Sure ?')  }}')" wire:click="delete({{$category->id}})">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
